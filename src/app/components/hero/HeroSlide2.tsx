...
export function HeroSlide2({
  ...
  const t  = useTranslations("hero.slide2");
  ...
  return (
    <div style={{ width: "100%", position: "relative", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: containerPadding, display: "flex", flexDirection: isStacked ? "column" : "row", alignItems: isStacked ? "flex-start" : "center", gap, width: "100%", position: "relative", zIndex: 2, flex: isStacked ? undefined : 1 }}>
        ...
        {/* Desktop human */}
        {!isStacked && (
          <div style={{ flex: "1 1 auto", position: "relative", height: "calc(100svh - 232px)", minHeight: 480, overflow: "visible" }}>
            ...
            <img
              src="/Slide2Human.avif"
              alt="Financial Specialist"
              fetchPriority="high"
              style={{ position: "absolute", bottom: 55, left: isSmallDesktop ? "-6%" : "-7%", height: isSmallDesktop ? "clamp(300px, 68vh, 460px)" : "calc(80%)", width: "auto", maxWidth: "none", objectFit: "contain", objectPosition: "bottom center", userSelect: "none", pointerEvents: "none", zIndex: 2 }}
            />
          </div>
        )}
      </div>
      ...
      {/* Stacked graphic */}
      {showGraphic && (
        <div style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "flex-start", overflow: "visible", position: "relative", zIndex: 1, minHeight: 0 }}>
          <div style={{ position: "absolute", bottom: 2, left: "50%", transform: "translateX(-50%)", width: 200, height: 16, borderRadius: "50%", background: "rgba(0,0,0,0.09)", filter: "blur(10px)", pointerEvents: "none" }} />
          <img src="/Slide2Human.avif" alt="" aria-hidden="true" style={{ height: isTablet ? 340 : isCompactPhone ? 264 : 300, width: "auto", objectFit: "contain", objectPosition: "bottom center", filter: "drop-shadow(-4px 0 14px rgba(0,0,0,0.15))", display: "block", position: "relative", zIndex: 1, userSelect: "none" }} />
        </div>
      )}
