"use client";

import { useEffect, useRef, useState } from "react";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import { Award, Eye, ShieldCheck, BarChart3 } from "lucide-react";
import { useBreakpoint } from "../../../hooks/useBreakpoint";

gsap.registerPlugin(ScrollTrigger);

const DARK = "#1A3D2B";
const ACCENT = "#52B788";
const BG = "#F5F4F1";

const values = [
  {
    Icon: Award,
    number: "01",
    title: "Peşəkarlıq",
    description:
      "Hər bir qərar dərin analitik təhlil və illərin təcrübəsinə əsaslanır. Ən mürəkkəb maliyyə məsələlərində də dəqiq həllər təqdim edirik.",
  },
  {
    Icon: Eye,
    number: "02",
    title: "Şəffaflıq",
    description:
      "Fəaliyyətimizi tam açıqlıq və qanunvericiliyə tam uyğunluq prinsipləri ilə aparırıq. Hesabat sistemimizdən heç nə gizli deyil.",
  },
  {
    Icon: ShieldCheck,
    number: "03",
    title: "Etibarlılıq",
    description:
      "Müştərilər bizə biznesin ən vacib maliyyə proseslərini etibar edir. Biz bu etimadı hər zaman qoruyuruq.",
  },
  {
    Icon: BarChart3,
    number: "04",
    title: "Səmərəlilik",
    description:
      "Tövsiyələrimiz yalnız nəzəri deyil, real biznes nəticələrini təmin edən praktik həllərə yönəlibdir.",
  },
];

export function AboutValues() {
  const bp = useBreakpoint();
  const isMobile = bp === "mobile";
  const isTablet = bp === "tablet";
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);

  const sectionRef = useRef<HTMLDivElement>(null);
  const progressBarRef = useRef<HTMLDivElement>(null);

  // Per-card refs
  const borderRefs = useRef<(HTMLDivElement | null)[]>([]);
  const headerRowRefs = useRef<(HTMLDivElement | null)[]>([]);
  const descRefs = useRef<(HTMLDivElement | null)[]>([]);
  const ghostRefs = useRef<(HTMLSpanElement | null)[]>([]);
  const iconBoxRefs = useRef<(HTMLDivElement | null)[]>([]);
  const iconFilterRefs = useRef<(HTMLDivElement | null)[]>([]);
  const titleRefs = useRef<(HTMLHeadingElement | null)[]>([]);
  const numLabelRefs = useRef<(HTMLSpanElement | null)[]>([]);
  const dotRefs = useRef<(HTMLDivElement | null)[]>([]);

  const activeIndexRef = useRef(-1);

  useEffect(() => {
    if (isMobile || !sectionRef.current) return;

    const ctx = gsap.context(() => {
      // ── Initial collapsed state for all ───────────────────────
      descRefs.current.forEach((d) => {
        if (d) gsap.set(d, { height: 0 });
      });
      borderRefs.current.forEach((b) => {
        if (b) gsap.set(b, { scaleY: 0, transformOrigin: "top center" });
      });
      ghostRefs.current.forEach((g) => {
        if (g) gsap.set(g, { opacity: 0 });
      });
      iconBoxRefs.current.forEach((box) => {
        if (box) gsap.set(box, { backgroundColor: "rgba(26,61,43,0.07)", boxShadow: "0 0 0 1.5px rgba(26,61,43,0.45)" });
      });
      iconFilterRefs.current.forEach((wrap) => {
        if (wrap) gsap.set(wrap, { clearProps: "filter" });
      });
      titleRefs.current.forEach((t) => {
        if (t) gsap.set(t, { color: "#9CA3AF" });
      });
      numLabelRefs.current.forEach((n) => {
        if (n) gsap.set(n, { color: "rgba(26,61,43,0.3)" });
      });
      dotRefs.current.forEach((d, i) => {
        if (d) gsap.set(d, { height: 8, backgroundColor: "rgba(26,61,43,0.18)" });
        void i;
      });
      gsap.set(progressBarRef.current, { scaleX: 0, transformOrigin: "left center" });

      // ── Open card 0 immediately via gsap.set (synchronous, no queuing) ──
      activeIndexRef.current = 0;
      if (descRefs.current[0])      gsap.set(descRefs.current[0],      { height: "auto" });
      if (borderRefs.current[0])    gsap.set(borderRefs.current[0],    { scaleY: 1, transformOrigin: "top center" });
      if (ghostRefs.current[0])     gsap.set(ghostRefs.current[0],     { opacity: 0.055 });
      if (iconBoxRefs.current[0])   gsap.set(iconBoxRefs.current[0],   { backgroundColor: DARK, boxShadow: `0 0 0 1.5px ${DARK}` });
      if (iconFilterRefs.current[0]) gsap.set(iconFilterRefs.current[0], { filter: "brightness(0) invert(1)" });
      if (titleRefs.current[0])     gsap.set(titleRefs.current[0],     { color: "#111410" });
      if (numLabelRefs.current[0])  gsap.set(numLabelRefs.current[0],  { color: DARK });
      if (dotRefs.current[0])       gsap.set(dotRefs.current[0],       { height: 24, backgroundColor: DARK });

      // ── Helpers ────────────────────────────────────────────────
      function snapClosed(i: number) {
        const targets = [
          descRefs.current[i], borderRefs.current[i], ghostRefs.current[i],
          iconBoxRefs.current[i], iconFilterRefs.current[i],
          titleRefs.current[i], numLabelRefs.current[i], dotRefs.current[i],
        ];
        targets.forEach((t) => { if (t) gsap.killTweensOf(t); });
        if (descRefs.current[i])      gsap.set(descRefs.current[i],      { height: 0 });
        if (borderRefs.current[i])    gsap.set(borderRefs.current[i],    { scaleY: 0, transformOrigin: "bottom center" });
        if (ghostRefs.current[i])     gsap.set(ghostRefs.current[i],     { opacity: 0 });
        if (iconBoxRefs.current[i])   gsap.set(iconBoxRefs.current[i],   { backgroundColor: "rgba(26,61,43,0.07)", boxShadow: "0 0 0 1.5px rgba(26,61,43,0.45)", scale: 1 });
        if (iconFilterRefs.current[i]) gsap.set(iconFilterRefs.current[i], { clearProps: "filter" });
        if (titleRefs.current[i])     gsap.set(titleRefs.current[i],     { color: "#9CA3AF" });
        if (numLabelRefs.current[i])  gsap.set(numLabelRefs.current[i],  { color: "rgba(26,61,43,0.3)" });
        if (dotRefs.current[i])       gsap.set(dotRefs.current[i],       { height: 8, backgroundColor: "rgba(26,61,43,0.18)" });
      }

      // ── Activate / deactivate ──────────────────────────────────
      function openCard(idx: number, prev: number) {
        // Snap any cards that were skipped (neither prev nor idx) to closed instantly
        values.forEach((_, i) => {
          if (i !== idx && i !== prev) snapClosed(i);
        });

        // Close previous with animation (kill stale tweens first)
        if (prev >= 0 && prev !== idx) {
          [descRefs, borderRefs, ghostRefs, iconBoxRefs, iconFilterRefs, titleRefs, numLabelRefs, dotRefs]
            .forEach((r) => { if (r.current[prev]) gsap.killTweensOf(r.current[prev]!); });
          gsap.to(descRefs.current[prev],    { height: 0, duration: 0.38, ease: "power2.inOut", overwrite: true });
          gsap.to(borderRefs.current[prev],  { scaleY: 0, duration: 0.32, ease: "power2.in", transformOrigin: "bottom center", overwrite: true });
          gsap.to(ghostRefs.current[prev],   { opacity: 0, duration: 0.24, overwrite: true });
          gsap.to(iconBoxRefs.current[prev], { backgroundColor: "rgba(26,61,43,0.07)", boxShadow: "0 0 0 1.5px rgba(26,61,43,0.45)", duration: 0.28, overwrite: true });
          if (iconFilterRefs.current[prev]) gsap.set(iconFilterRefs.current[prev], { clearProps: "filter" });
          gsap.to(titleRefs.current[prev],   { color: "#9CA3AF", duration: 0.28, overwrite: true });
          gsap.to(numLabelRefs.current[prev],{ color: "rgba(26,61,43,0.3)", duration: 0.28, overwrite: true });
          gsap.to(dotRefs.current[prev],     { height: 8, backgroundColor: "rgba(26,61,43,0.18)", duration: 0.26, overwrite: true });
        }

        // Open new (kill stale tweens first)
        [descRefs, borderRefs, ghostRefs, iconBoxRefs, iconFilterRefs, titleRefs, numLabelRefs, dotRefs]
          .forEach((r) => { if (r.current[idx]) gsap.killTweensOf(r.current[idx]!); });
        gsap.to(descRefs.current[idx],    { height: "auto", duration: 0.55, ease: "expo.out", overwrite: true });
        gsap.to(borderRefs.current[idx],  { scaleY: 1, duration: 0.52, ease: "expo.out", transformOrigin: "top center", overwrite: true });
        gsap.to(ghostRefs.current[idx],   { opacity: 0.055, duration: 0.55, overwrite: true });
        if (iconFilterRefs.current[idx]) gsap.set(iconFilterRefs.current[idx], { filter: "brightness(0) invert(1)" });
        gsap.to(titleRefs.current[idx],   { color: "#111410", duration: 0.34, overwrite: true });
        gsap.to(numLabelRefs.current[idx],{ color: DARK, duration: 0.34, overwrite: true });
        gsap.to(dotRefs.current[idx],     { height: 24, backgroundColor: DARK, duration: 0.36, ease: "back.out(1.5)", overwrite: true });
        gsap.fromTo(iconBoxRefs.current[idx], { scale: 0.82 }, { scale: 1, backgroundColor: DARK, boxShadow: `0 0 0 1.5px ${DARK}`, duration: 0.48, ease: "back.out(2)", overwrite: true });
      }

      // ── ScrollTrigger pin ──────────────────────────────────────
      ScrollTrigger.create({
        trigger: sectionRef.current,
        pin: true,
        start: "top top",
        end: "+=300%",
        scrub: 1.5,
        onUpdate(self) {
          gsap.set(progressBarRef.current, { scaleX: self.progress });

          const idx = Math.min(Math.floor(self.progress * 4), 3);
          if (idx !== activeIndexRef.current) {
            const prev = activeIndexRef.current;
            activeIndexRef.current = idx;
            openCard(idx, prev); // openCard no longer takes an `instant` arg
          }
        },
      });

      // Header entrance
      gsap.fromTo(
        ".val-hdr",
        { opacity: 0, y: 20 },
        {
          opacity: 1,
          y: 0,
          duration: 0.85,
          ease: "power2.out",
          scrollTrigger: {
            trigger: sectionRef.current,
            start: "top 84%",
            once: true,
          },
        }
      );
    }, sectionRef);

    return () => ctx.revert();
  }, [isMobile, isTablet, mounted]);

  // ── Mobile / Tablet (or before mount — prevents GSAP from firing on wrong layout) ──
  if (!mounted || isMobile || isTablet) {
    return (
      <section
        style={{
          backgroundColor: BG,
          padding: "56px 20px 64px",
          fontFamily: "'Inter', sans-serif",
        }}
      >
        {/* Header */}
        <p style={{
          fontSize: 10, fontWeight: 700, color: DARK,
          letterSpacing: "0.22em", textTransform: "uppercase", margin: "0 0 12px",
        }}>
          Dəyərlərimiz
        </p>
        <h2 style={{
          fontFamily: "'Plus Jakarta Sans', sans-serif",
          fontWeight: 800,
          fontSize: "clamp(24px, 7.5vw, 30px)",
          color: "#111410",
          margin: "0 0 32px",
          letterSpacing: "-0.035em",
          lineHeight: 1.1,
        }}>
          4 dəyər,{" "}
          <span style={{ color: DARK }}>1 standart</span>
        </h2>

        {/* Single-column card list */}
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {values.map((v, i) => (
            <div
              key={i}
              style={{
                backgroundColor: "#ffffff",
                borderRadius: 16,
                padding: "20px 18px",
                display: "flex",
                flexDirection: "row",
                gap: 16,
                alignItems: "flex-start",
                borderLeft: `3px solid ${DARK}`,
              }}
            >
              {/* Icon */}
              <div style={{
                width: 42, height: 42, borderRadius: 11,
                backgroundColor: `${DARK}10`,
                border: `1.5px solid ${DARK}20`,
                display: "flex", alignItems: "center", justifyContent: "center",
                flexShrink: 0,
              }}>
                <v.Icon size={18} color={DARK} strokeWidth={1.6} />
              </div>

              {/* Text */}
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  <span style={{
                    fontSize: 10, fontWeight: 700, color: DARK,
                    letterSpacing: "0.12em", opacity: 0.45,
                  }}>
                    {v.number}
                  </span>
                  <h3 style={{
                    fontFamily: "'Plus Jakarta Sans', sans-serif",
                    fontWeight: 700,
                    fontSize: 15,
                    color: "#111410",
                    margin: 0,
                    letterSpacing: "-0.02em",
                    lineHeight: 1.25,
                  }}>
                    {v.title}
                  </h3>
                </div>
                <p style={{ fontSize: 13, color: "#6B7280", lineHeight: 1.7, margin: 0 }}>
                  {v.description}
                </p>
              </div>
            </div>
          ))}
        </div>
      </section>
    );
  }

  // ── Desktop / Tablet ──────────────────────────────────────────
  const hPad = isTablet ? "0 32px" : "0 64px";

  return (
    <section
      ref={sectionRef}
      style={{
        backgroundColor: BG,
        position: "relative",
        overflow: "hidden",
      }}
    >
      <div
        style={{
          minHeight: "100vh",
          display: "flex",
          flexDirection: "column",
          justifyContent: "center",
          padding: hPad,
          maxWidth: 1280,
          margin: "0 auto",
          position: "relative",
        }}
      >
        {/* ── Section header ── */}
        <div
          className="val-hdr"
          style={{ marginBottom: isTablet ? 36 : 52 }}
        >
          <p
            style={{
              fontSize: 10,
              fontWeight: 700,
              color: DARK,
              letterSpacing: "0.22em",
              textTransform: "uppercase",
              margin: "0 0 14px",
            }}
          >
            Dəyərlərimiz
          </p>
          <div
            style={{
              display: "flex",
              alignItems: "flex-end",
              justifyContent: "space-between",
              gap: 32,
              flexWrap: "wrap",
            }}
          >
            <h2
              style={{
                fontFamily: "'Plus Jakarta Sans', sans-serif",
                fontWeight: 800,
                fontSize: isTablet ? 36 : "clamp(36px, 4vw, 54px)",
                color: "#111410",
                margin: 0,
                letterSpacing: "-0.04em",
                lineHeight: 1.06,
              }}
            >
              4 dəyər,{" "}
              <span style={{ color: DARK }}>1 standart</span>
            </h2>
            <p
              style={{
                fontSize: 15,
                color: "#9CA3AF",
                lineHeight: 1.72,
                margin: 0,
                maxWidth: 310,
                fontFamily: "'Inter', sans-serif",
              }}
            >
              Hər müştəri üçün eyni əsaslar üzərində işləyirik — kiçik
              sahibkardan iri korporasiyaya qədər.
            </p>
          </div>
        </div>

        {/* ── Card list ── */}
        <div style={{ position: "relative" }}>
          {values.map((v, i) => (
            <div
              key={i}
              style={{
                position: "relative",
                borderTop: "1px solid #E0DDD8",
              }}
            >
              {/* Left green accent border */}
              <div
                ref={(el) => { borderRefs.current[i] = el; }}
                style={{
                  position: "absolute",
                  left: 0,
                  top: 0,
                  bottom: 0,
                  width: 3,
                  borderRadius: 2,
                  backgroundColor: DARK,
                  transform: "scaleY(0)",
                  transformOrigin: "top center",
                  zIndex: 2,
                }}
              />

              {/* Ghost large number */}
              <span
                ref={(el) => { ghostRefs.current[i] = el; }}
                style={{
                  position: "absolute",
                  right: isTablet ? -8 : -20,
                  top: "50%",
                  transform: "translateY(-50%)",
                  fontFamily: "'Plus Jakarta Sans', sans-serif",
                  fontWeight: 900,
                  fontSize: isTablet ? 108 : 148,
                  color: DARK,
                  opacity: 0,
                  letterSpacing: "-0.06em",
                  lineHeight: 1,
                  userSelect: "none",
                  pointerEvents: "none",
                  zIndex: 0,
                }}
              >
                {v.number}
              </span>

              {/* Always-visible header row */}
              <div
                ref={(el) => { headerRowRefs.current[i] = el; }}
                style={{
                  display: "flex",
                  alignItems: "center",
                  gap: isTablet ? 20 : 32,
                  padding: isTablet ? "22px 0 22px 16px" : "26px 0 26px 16px",
                  position: "relative",
                  zIndex: 1,
                }}
              >
                {/* Number label */}
                <span
                  ref={(el) => { numLabelRefs.current[i] = el; }}
                  style={{
                    fontFamily: "'Plus Jakarta Sans', sans-serif",
                    fontWeight: 800,
                    fontSize: 12,
                    color: "rgba(26,61,43,0.3)",
                    letterSpacing: "0.12em",
                    flexShrink: 0,
                    width: isTablet ? 36 : 44,
                  }}
                >
                  {v.number}
                </span>

                {/* Icon */}
                <div
                  ref={(el) => { iconBoxRefs.current[i] = el; }}
                  style={{
                    width: isTablet ? 44 : 50,
                    height: isTablet ? 44 : 50,
                    borderRadius: 13,
                    flexShrink: 0,
                    boxShadow: "0 0 0 1.5px rgba(26,61,43,0.45)",
                    backgroundColor: "rgba(26,61,43,0.07)",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                  }}
                >
                  <div
                    ref={(el) => { iconFilterRefs.current[i] = el; }}
                    style={{ display: "flex" }}
                  >
                    <v.Icon size={isTablet ? 19 : 21} color={DARK} strokeWidth={1.6} />
                  </div>
                </div>

                {/* Title */}
                <h3
                  ref={(el) => { titleRefs.current[i] = el; }}
                  style={{
                    fontFamily: "'Plus Jakarta Sans', sans-serif",
                    fontWeight: 700,
                    fontSize: isTablet ? 20 : 23,
                    color: "#9CA3AF",
                    margin: 0,
                    letterSpacing: "-0.03em",
                    lineHeight: 1.2,
                  }}
                >
                  {v.title}
                </h3>
              </div>

              {/* Expandable description — aligned under icon */}
              <div
                ref={(el) => { descRefs.current[i] = el; }}
                style={{ overflow: "hidden", height: 0 }}
              >
                <p
                  style={{
                    fontSize: isTablet ? 14 : 15,
                    color: "#6B7280",
                    lineHeight: 1.78,
                    margin: 0,
                    fontFamily: "'Inter', sans-serif",
                    maxWidth: 580,
                    paddingLeft: isTablet ? 100 : 126,
                    paddingBottom: 26,
                    paddingTop: 2,
                  }}
                >
                  {v.description}
                </p>
              </div>
            </div>
          ))}

          {/* Bottom border of last card */}
          <div style={{ height: 1, backgroundColor: "#E0DDD8" }} />
        </div>
      </div>

      {/* ── Progress dots ── */}
      <div
        style={{
          position: "absolute",
          right: isTablet ? 14 : 24,
          top: "50%",
          transform: "translateY(-50%)",
          display: "flex",
          flexDirection: "column",
          alignItems: "center",
          gap: 10,
          zIndex: 20,
        }}
      >
        {values.map((_, i) => (
          <div
            key={i}
            ref={(el) => { dotRefs.current[i] = el; }}
            style={{
              width: 3,
              height: 8,
              borderRadius: 4,
              backgroundColor: "rgba(26,61,43,0.18)",
            }}
          />
        ))}
      </div>

      {/* ── Bottom progress bar ── */}
      <div
        style={{
          position: "absolute",
          bottom: 0,
          left: 0,
          right: 0,
          height: 2,
          backgroundColor: "rgba(26,61,43,0.06)",
          zIndex: 20,
          overflow: "hidden",
        }}
      >
        <div
          ref={progressBarRef}
          style={{
            height: "100%",
            width: "100%",
            background: `linear-gradient(90deg, ${DARK}, ${ACCENT})`,
            transformOrigin: "left center",
          }}
        />
      </div>
    </section>
  );
}
